...
catAudioRand = shuffle(Array.from(Array(catAudio.length).keys()));
var catAudio = [1,2];
var zipfFreq = [8,4,2,1];
//var zipfFreq = [6,2,1,1];
var unfFreq = [4,4,4,3];
var mixedFreq = [[8,4,2,1],[4,4,4,3]]
...
for (i=0;i<furnitureInd.length;i++) //each Ind is a unique category item
{
  for (j=0; j<furnitureFreq[i];j++)
  {
    ...
    furnitureTrain.push([furnitureInd[i],catAudioRand[0]]); //same random sound assigned at every index.
  }
}
...
for (i=0;i<carsInd.length;i++) //each Ind is a unique category item
{
  ...
  {
    carTrain.push([carsInd[i],catAudioRand[1]]); //same random sound assigned at every index.
  }
}
...
for (i=0;i<kitchenInd.length;i++) //each Ind is a unique category item
{
  ...
  {
    kitchenTrain.push([kitchenInd[i],catAudioRand[2]]); //same random sound assigned at every index.
  }
}
...
for (i=0;i<dogInd.length;i++) //each Ind is a unique category item
{
  ...
  {
    dogTrain.push([dogInd[i],catAudioRand[3]]); //same random sound assigned at every index.
  }
}
...
for (i=0;i<insectInd.length;i++) //each Ind is a unique category item
{
  ...
  {
    insectTrain.push([insectInd[i],catAudioRand[4]]); //same random sound assigned at every index.
  }
}
...
for (i=0;i<clothesInd.length;i++) //each Ind is a unique category item
{
  ...
  {
    clothesTrain.push([clothesInd[i],catAudioRand[5]]); //same random sound assigned at every index.
  }
}
...
rfull=r3.concat(r); // for 45 trial version

rfull=r3 // for the 30 trial version

// r2=r.reduce(function (res, current, index, array) {
//     return res.concat([current, current]);



// farray=Array.from({length: (furnitureTrain.length)}, () => Math.floor(Math.random() * 1)) //array of all 0's
// carray=Array.from({length: (carTrain.length)}, () => Math.floor(Math.random() * 1)+1) //array of all 1's
// karray=Array.from({length: (kitchenTrain.length)}, () => Math.floor(Math.random() * 1)+2) //array of all 2's
// darray=Array.from({length: (dogTrain.length)}, () => Math.floor(Math.random() * 1)+3) //array of all 3's
// iarray=Array.from({length: (insectTrain.length)}, () => Math.floor(Math.random() * 1)+4) //array of all 4's
// clarray=Array.from({length: (clothesTrain.length)}, () => Math.floor(Math.random() * 1)+5) //array of all 5's
//
// combine=[farray,carray,karray,darray,iarray,clarray]



train_simul_image=2; //how many images are shown at once during training
